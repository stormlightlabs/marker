...
if (markerWindow.__markerContentRuntimeLoaded !== true) {
  markerWindow.__markerContentRuntimeLoaded = true;

  installAnnotationToolbar();
  const visibilityStyle = document.createElement('style');
  visibilityStyle.textContent =
    'html[data-marker-annotations-hidden] .marker-annotation { background: transparent !important; text-decoration: none !important; }';
  ...
  chrome.runtime.onMessage.addListener((message: unknown, _sender, sendResponse) => {
    ...
    if (candidate.type === MarkerMessageType.ContentRuntimeStatus) {
